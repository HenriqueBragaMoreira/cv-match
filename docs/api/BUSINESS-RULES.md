...
- **Status**: `implemented`
- **Scope (api)**: The improvement prompt explicitly instructs the AI not to fabricate experience or skills. The prompt includes the original resume as the source of truth for factual content. Enforced via `CV_IMPROVEMENT_SYSTEM_PROMPT` in `src/prompts/cv-improvement.ts`.

### BR-007: Improvement transparency
- **Status**: `implemented`
- **Scope (api)**: The `/improve` endpoint returns a `changes` summary describing what was modified, generated by the AI alongside the improved resume.
